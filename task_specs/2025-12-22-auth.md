...
- [x] add an rpc to the users service which validates a password
    ...
    - [x] update the validate password route to use the username instead of the userId 
        - [x] make sure that usernames are globally unique
- [x] add a route to the api gateway which takes the credentials and then calls the credential validation rpc on the users service
    ...
    - [x] implement the route in the api gateway service layer:
        - [x] call the user service to validate credentials
        - [x] use a function defined in a package to generate a token
    - [x] ensure that the login route can distinguish between a user that does not exist and an incorrect password
- [x] implement authentication middleware on all of the api gateway routes except for the login route
    - [x] add a middleware that parses and validates the token